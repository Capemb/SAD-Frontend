    ...
    <main class="content">
        ...
        <section class="kpis" aria-label="Indicadores">
          <transition-group name="stagger" tag="div" class="kpi-grid">
            <div class="kpi-card" v-for="(k, idx) in kpis" :key="k.title" :style="{ animationDelay: (idx * 80) + 'ms' }">
              <div class="kpi-left">
        ...
        <section class="grid-2">
          ...
          <div class="card modules-card">
            <div class="card-header">
              ...
              <div>
                <button class="btn" @click="openCreateModule">Criar módulo</button>
              </div>
            </div>
              ...
              <ul v-else class="modules-list">
                <li v-for="m in modules" :key="m.id" class="module-item">
                  ...
                  <div class="module-actions">
                    <button class="icon" title="Editar" @click="editModule(m)"><i class="fa-solid fa-pen"></i></button>
                    <button class="icon danger" title="Remover" @click="removeModule(m)"><i class="fa-solid fa-trash"></i></button>
                  </div>
                </li>
              </ul>
            </div>
          ...
          <!-- Ciclos / Relatórios card -->
          <div class="card cycles-card">
            <div class="card-header">
              <h3>Ciclos Recentes</h3>
              <div class="muted small">Visão rápida do último ciclo</div>
            </div>

            <div class="card-body">
              <table class="cycles-table" aria-label="Ciclos de avaliação">
                <thead>
                  <tr>
                    <th>Nome</th>
                    <th>Período</th>
                ...
                <tbody>
                  <tr v-for="c in cycles" :key="c.id">
                    ...
                    <td><span :class="['status', c.status]">{{ c.statusLabel }}</span></td>
                    <td>
                      <div class="progress">
                        <div class="bar" :style="{ width: c.progress + '%' }"></div>
                      </div>
                      <small class="muted">{{ c.progress }}%</small>
                    </td>
                    <td><button class="btn ghost" @click="viewCycle(c)">Ver</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <section class="grid-3">
          <div class="card quick">
            ...
            <div class="quick-grid">
              <button class="quick-btn" @click="openCreateModule"><i class="fa-solid fa-folder-plus"></i> Novo Módulo</button>
              <button class="quick-btn" @click="openCreateCycle"><i class="fa-solid fa-calendar-plus"></i> Novo Ciclo</button>
              <button class="quick-btn" @click="openReports"><i class="fa-solid fa-file-export"></i> Exportar Relatório</button>
            </div>
          </div>

          <div class="card chart-card">
            <h4>Distribuição por departamento</h4>
            <div class="chart-placeholder" role="img" aria-label="Gráfico de pizza">
              <svg viewBox="0 0 200 100" preserveAspectRatio="none" class="chart-svg">
                <rect v-for="(b,i) in chartData" :key="i" :x="i*40 + 10" :y="100 - b" width="20" :height="b" rx="3"></rect>
              </svg>
            </div>
          </div>
          ...
          <div class="card report-card">
            ...
            <ul class="reports-list">
              <li v-for="r in reports" :key="r.id">
                <div><strong>{{ r.title }}</strong><div class="muted small">{{ r.date }}</div></div>
                <div><button class="btn ghost" @click="downloadReport(r)"><i class="fa-solid fa-download"></i></button></div>
              </li>
            </ul>
          </div>
        </section>
    ...
    <transition name="modal-fade">
      <div v-if="showModuleModal" class="modal-overlay" @click.self="closeModuleModal">
        <div class="modal-content" role="dialog" aria-modal="true">
          <h3>{{ editingModule ? 'Editar Módulo' : 'Criar Módulo' }}</h3>

          <form @submit.prevent="saveModule">
            <label>Nome</label>
            <input v-model="moduleForm.nome" required maxlength="100" />
